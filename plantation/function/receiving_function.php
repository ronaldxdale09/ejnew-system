...
if (isset($_POST['add'])) {
    ...
    $weight = str_replace(',', '', $_POST['weight'] ?? '0');
    $reweight = str_replace(',', '', $_POST['reweight'] ?? '0');
...
if (isset($_POST['update'])) {
    ...
    $ru_weight = str_replace(',', '', $_POST['ru_weight'] ?? '0');
    $ru_reweight = str_replace(',', '', $_POST['ru_reweight'] ?? '0');
...
if (isset($_POST['delete'])) {
    ...
    if ($type === 'EJN') {
        $restoreSql = "UPDATE ejn_rubber_transfer SET planta_status = '1' WHERE ejn_id = '$purchased_id'";
    } elseif ($type === 'DRY') {
        $restoreSql = "UPDATE dry_price_transfer SET planta_status = '1' WHERE dry_id = '$purchased_id'";
    } else {
        $restoreSql = "UPDATE rubber_transaction SET planta_status = '1' WHERE id = '$purchased_id'";
    }

    $query = "DELETE FROM planta_recording WHERE recording_id = '$recording_id'";

    if (mysqli_query($con, $query)) {
        mysqli_query($con, $restoreSql);
        header('Location: ../recording.php');
