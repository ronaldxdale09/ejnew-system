...
$location = $_POST['container_loc'];
  ...
  total_cuplump_weight = '$totalWeight', total_cuplump_cost = '$totalCost', ave_cuplump_cost = '$averageCost',status='Draft'
...
if (mysqli_query($con, $query)) {
    ...
    if (isset($_POST["cuplump_cost"])) {
        ...
        foreach ($type as $index => $costType) {
            ...
            $sql = "INSERT INTO `sales_cuplump_container_inv`(`sales_cuplump_id`, `supplier`, `location`, `loading_weight`, `cost_type`, `wet_cost`, `dry_cost`, `drc`, `cuplump_cost`) 
            VALUES ('$container_id','$suppliers','$location','$loading_weight','$costType','$wet_cost','$dry_cost','$drcInput','$cuplump_cost')";
