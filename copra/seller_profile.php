...
$results = mysqli_query($con, "SELECT * from copra_seller WHERE code='$id'"); 
$row = mysqli_fetch_array($results);
$sellerID = $row['id'];
$name = $row['name'];

   //PENDING CONTRACT
   $pendingContract_count = mysqli_query($con,"SELECT * FROM copra_contract where status='PENDING' OR status='UPDATED' AND copra_seller='$name'");
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card mb-3">
                                <div class="card-body">
                                    ...
                                    $results  = mysqli_query($con, "SELECT * from copra_contract WHERE copra_seller='$name' "); 
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table" id='seller_copraTransaction'>
                                            <?php
                                    $results  = mysqli_query($con, "SELECT * from copra_transaction where copra_seller='$name'  ORDER BY id DESC  "); ?>
                                            <thead class="table-dark">
                                                <tr>
...
<?php
            $sql = mysqli_query($con,"SELECT SUM(amount) as total FROM copra_cashadvance WHERE copra_seller='$name' and category='copra'");
            $copra_row=mysqli_fetch_array($sql);

            $sql = mysqli_query($con,"SELECT SUM(amount) as total  FROM copra_cashadvance WHERE copra_seller='$name' and category='ntc'");
            $ntc_row=mysqli_fetch_array($sql);
            $sql = mysqli_query($con,"SELECT SUM(amount) as total  FROM copra_cashadvance WHERE copra_seller='$name' and category='trucking'");
            $truck_row=mysqli_num_rows($sql);
            $sql = mysqli_query($con,"SELECT SUM(amount) as total  FROM copra_cashadvance WHERE copra_seller='$name' and category='other'");
...
<?php
   ...
   $today = $currentYear . "-" . $currentMonth . "-" . $currentDay;
                ...
                $purchased_count = mysqli_query($con,"SELECT year(date) as year ,MONTHNAME(date) as monthname,sum(net_res) as month_total from copra_transaction WHERE year(date)='$currentYear' and copra_seller='$name'  group by month(date) ORDER BY date");        
