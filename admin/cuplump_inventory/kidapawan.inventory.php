...
$sql = mysqli_query($con, "SELECT SUM(reweight) as inventory from  planta_recording where status='Field' AND planta_recording.source='Kidapawan'   ");
$cuplumps_Kidapawan = mysqli_fetch_array($sql);

$sql = mysqli_query($con, "SELECT SUM(crumbed_weight) as inventory from  planta_recording where status='Milling'  AND planta_recording.source='Kidapawan'   ");
$milling_Kidapawan = mysqli_fetch_array($sql);


$sql = mysqli_query($con, "SELECT SUM(dry_weight) as inventory from  planta_recording where status='Drying' AND planta_recording.source='Kidapawan'    ");
$drying_Kidapawan = mysqli_fetch_array($sql);
        ...
        <div class="stat-card">
            <div class="stat-card__content">
                ...
                <h3>
                    ...
                    <?php echo number_format($cuplumps_Kidapawan['inventory'] ?? 0, 0) ?> kg
                </h3>
                <div>
    ...
    <div class="col">
        <div class="stat-card">
            <div class="stat-card__content">
                ...
                <h3>
                    <i class="text-danger font-weight-bold mr-1"></i>
                    <?php echo number_format($milling_Kidapawan['inventory'] ?? 0, 0) ?> kg
                </h3>
    ...
    <div class="col">
        <div class="stat-card">
            <div class="stat-card__content">
                ...
                <h3>
                    <i class="text-danger font-weight-bold mr-1"></i>
                    <?php echo number_format($drying_Kidapawan['inventory'] ?? 0, 0) ?> kg
                </h3>
...
<hr>
<table class="table table-bordered table-hover table-striped" id='inventory-table-Kidapawan' style="width:100%">
    <?php
    $results  = mysqli_query($con, "SELECT DISTINCT planta_recording.*, rubber_transaction.total_amount as total_amount, rubber_transaction.net_weight as net_weight 
                                    ...
                                    WHERE planta_recording.status = 'Field' AND planta_recording.source='Kidapawan' "); ?>
...
<script>
    $(document).ready(function() {
        var table = $('#inventory-table-Kidapawan').DataTable({
            "order": [
                [1, 'asc']
            ],
            "pageLength": -1,
