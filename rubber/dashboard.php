...
<?php
   ...
   $Bales_currentMonth = date("m");
                $bales_count = mysqli_query($con,"SELECT * ,MONTHNAME(date) as monthname from bales_transaction group by month(date)");        
                if($bales_count->num_rows > 0) {
                  foreach($bales_count as $b_data) {
                    
                      $total_bale_1 = $b_data['net_weight_1'] /  $b_data['kilo_bales_1'];
                      $total_bale_2 = $b_data['net_weight_2'] /  $b_data['kilo_bales_2']; 
                      $bales[] = $total_bale_1+ $total_bale_2;
