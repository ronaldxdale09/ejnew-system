...
if (isset($_POST['coffee_sale_update'])) {
    // Retrieve the form data
    $coffeeSaleId = $_POST['coffee_sale_id'];
    ...
    $existingCoffeeSales = array();

    $fetchSql = "SELECT sale_line_id FROM coffee_sale_line WHERE coffee_sale_id = '$coffeeSaleId'";
    $fetchResult = mysqli_query($con, $fetchSql);
    if (!$fetchResult) {
        die('Error fetching existing bales: ' . mysqli_error($con));
    } else {
        while ($row = mysqli_fetch_assoc($fetchResult)) {
            $existingBales[] = $row['sale_line_id'];
        }
    }
