...
// Current month data
$total_transactions_curr_month = mysqli_fetch_array(mysqli_query($con, "SELECT COUNT(*) as count FROM ledger_maloong WHERE MONTH(date) = $curr_month AND YEAR(date) = $curr_year"))['count'];
$total_net_kilos_curr_month = mysqli_fetch_array(mysqli_query($con, "SELECT SUM(net_kilos) as sum_kilos FROM ledger_maloong WHERE MONTH(date) = $curr_month AND YEAR(date) = $curr_year"))['sum_kilos'];
$total_ejn_revenue_curr_month = mysqli_fetch_array(mysqli_query($con, "SELECT SUM(ejn_total) as ejn_total FROM ledger_maloong WHERE MONTH(date) = $curr_month AND YEAR(date) = $curr_year"))['ejn_total'];
$total_topper_gross_curr_month = mysqli_fetch_array(mysqli_query($con, "SELECT SUM(topper_gross) as topper_gross FROM ledger_maloong WHERE MONTH(date) = $curr_month AND YEAR(date) = $curr_year"))['topper_gross'];

// Previous month data
$total_transactions_prev_month = mysqli_fetch_array(mysqli_query($con, "SELECT COUNT(*) as count FROM ledger_maloong WHERE MONTH(date) = $prev_month AND YEAR(date) = $prev_year"))['count'];
$total_net_kilos_prev_month = mysqli_fetch_array(mysqli_query($con, "SELECT SUM(net_kilos) as sum_kilos FROM ledger_maloong WHERE MONTH(date) = $prev_month AND YEAR(date) = $prev_year"))['sum_kilos'];
$total_ejn_revenue_prev_month = mysqli_fetch_array(mysqli_query($con, "SELECT SUM(ejn_total) as ejn_total FROM ledger_maloong WHERE MONTH(date) = $prev_month AND YEAR(date) = $prev_year"))['ejn_total'];
$total_topper_gross_prev_month = mysqli_fetch_array(mysqli_query($con, "SELECT SUM(topper_gross) as topper_gross FROM ledger_maloong WHERE MONTH(date) = $prev_month AND YEAR(date) = $prev_year"))['topper_gross'];
